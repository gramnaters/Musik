  ...
  return (
    <ContextMenu>
      ...
      <ContextMenuContent className="w-56 bg-popover border-border">
        ...
        <ContextMenuSeparator />
        <ContextMenuItem
          onClick={() => useDownloadStore.getState().openDownload(track)}
          className="text-foreground focus:bg-accent focus:text-foreground"
        >
          Download
        </ContextMenuItem>
  ...
  const qualityEl = badge ? (
    <Tooltip delayDuration={200}>
      <TooltipTrigger asChild>
        ...
        >
          {badge.label === 'ATMOS' ? (
            <svg viewBox="0 0 24 24" fill="currentColor" className="w-3 h-3 text-white/80">
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-56 bg-popover border-border" onClick={(e) => e.stopPropagation()}>
        ...
        <DropdownMenuSeparator />
        <DropdownMenuItem onClick={() => useDownloadStore.getState().openDownload(track)}>Download</DropdownMenuItem>
        {track.streamURL && (
